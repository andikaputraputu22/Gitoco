 ...
 *
 * Works on a local draft of `PortfolioEdits`; every keystroke is pushed to the
 * parent as a live draft so the preview updates immediately. Save commits the
 * draft to the app state (the single source of truth for preview, public
 * portfolio and PDF), Cancel restores the previously saved edits.
 */
export function PortfolioEditor({
  ...
  function setProject(id: string, patch: Partial<{ title: string; blurb: string; tech: string[]; hidden: boolean }>) {
    apply({
      ...draft,
    ...
    >
      ...
      >
        <SheetHeader className="px-6 pt-6">
          ...
          <SheetDescription>
            Refine the generated content. Changes update the preview instantly and are used by the
            public portfolio and PDF export once saved.
          </SheetDescription>
        </SheetHeader>
            ...
            <div className="flex flex-wrap gap-1.5">
              {skills.map((s) => (
                <span
                  ...
                  data-testid={`editor-skill-${s.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`}
                >
                  {s}
                  <button
                    type="button"
                    ...
                    data-testid={`editor-skill-remove-${s.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`}
                  >
                    <X className="h-3 w-3" />
                  </button>
                </span>
        ...
        <SheetFooter className="sticky bottom-0 gap-2 border-t border-border bg-popover px-6 py-4">
